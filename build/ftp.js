...
function _getDirs (dir) {
  ...
  list.forEach(file => {
    ...
    if (st && st.isDirectory()) {
      ...
      let dir =
        path.parse(pathToFile).dir === config.build.assetsRoot
          ? ''
...
function _cleanServer (callback) {
  client.list('/public_html', false, (err, list) => {
    if (!err) {
      log(list)
    } else {
      log(err)
...
function _uploadFiles (filesList) {
  filesList.forEach(file => {
    ...
    client.put(file, serverFilePath, err => {
      if (!err) {
        log(
          chalk.blue(`File ${pathFile} uploaded to ${serverFilePath} correctly`)
        )
      } else {
        log(chalk.magenta(`${err}`))
      }
...
client.on('ready', () => {
  log(
    chalk.green(
      `Connection to ${ftpConfig.host}:${ftpConfig.port}@${
        ftpConfig.user
      } started succesfully`
    )
  )
